...
const btn2 = document.querySelector("#my_btn2");
btn2.addEventListener("click", function (event) {
    if (btn2.innerHTML === "Hide") {
        // con.innerHTML = "";
...
console.log(`records_per_page is: ${records_per_page}`);
///////////////////////////////////////////////////////////
let recordsppage = document.querySelector("#recordsppage");

recordsppage.addEventListener("keyup", function (event) {

    records_per_page = event.target.value;
    console.log(`records_per_page is: ${records_per_page}`);

    if (records_per_page == null || records_per_page == "" || records_per_page == 0) {
        records_per_page = 10;
        changePage(1);
    } else {
        changePage(1);
...
function prevPage() {
    if (current_page > 1) {
        current_page--;
        changePage(current_page);
        console.log(`prevPage ${current_page}`);
    }
}
...
function nextPage() {
    if (current_page < numPages()) {
        current_page++;
        changePage(current_page);
        console.log(`nextPage ${current_page}`);
    }
}
...
function changePage(page) {
    ...
    for (let i = (page - 1) * records_per_page; i < (page * records_per_page); i++) {
        ...
        newDiv.innerHTML = `<h3> ${myJson[i].title}</h3><p>${myJson[i].body}</p>`
        con.appendChild(newDiv);
    }
    //////////////////////////////////////////////////////////////////
    page_span.innerHTML = `${page} of: ${numPages()}, records 1 to  10`;
...
function numPages() {
    return Math.ceil(myJson.length / records_per_page);
}
